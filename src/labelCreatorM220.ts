...
async function generateUrlAndQRCode(input: string,path:string): Promise<void> {
    ...
    // Generiere die URL
    const url = `https://aas.interop4x.de/?aas=https://aas-env.interop4x.de/shells/${base64Encoded}`;
...
export function createPdf(JobOrderId: string, outputPath: string, jobFile: string) {
    return new Promise((resolve, reject) => {
        ...
        doc.image('images/interop4X.png', 15, 2, {fit: [20, 20]})
        ...
        const centeredX = 26 - (textWidth / 2);
        ...
        const centeredX2 = 26 - (textWidth2 / 2);
        doc.text(tmp,centeredX2,45,{
            align: 'center',
        });
        ...
        fs.readFile(jobFile, 'utf8', (err: any, fileContent: string) => {
            ...
            doc.image("images/qr_"+JobOrderId+".png", 5, 49, {fit: [40, 40]})
